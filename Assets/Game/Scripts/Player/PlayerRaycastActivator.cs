...
public class PlayerRaycastActivator : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (_takingObject != null)
            {
                if(_takingObject.GetComponent<Sword>() != null)
                {
                    _takingObject.GetComponent<Sword>().Attack();
                }
                else
                {
                    _takingObject.DropMe();
                    _takingObject.GetComponent<Rigidbody>().velocity = Camera.main.transform.forward * 4f;
                    _takingObject = null;
                }
            }
